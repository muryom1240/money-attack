...
async function getTrendingContext() {
  ...
  if (!trendKeyword) {
    const fallbacks = [
      ...
      // ゲーム開発・3DCG系
      '個人ゲーム開発', 'ローポリ3Dモデル', 'Unity副業', 'インディーゲーム販売',
      '3Dモデル販売', 'ゲーム開発で稼ぐ', 'BOOTH販売', 'ゲームアセット',
      'Blender副業', 'UE5個人開発', '3DCG副業', 'デジタルコンテンツ販売',
    ];
...
7. まとめ`,
    ...
    {
      name: 'gamedev_guide',
      titleHint: 'ゲーム開発・3DCG制作で稼ぐ方法',
      structureGuide: `
【記事構成（ゲーム開発・3DCG副業形式）】
1. 冒頭：「個人でゲームアセットを売って稼げるの？」という疑問に答える（300文字）
2. h2: 「ゲーム開発・3DCGで稼ぐ仕組みをわかりやすく解説」
3. h2: 「初心者でも始めやすい具体的な方法3選」
   - h3: ① BOOTHで3Dモデル・アセットを販売する
   - h3: ② Unityアセットストアで販売する
   - h3: ③ Skebやコミッションで受注制作する
4. h2: 「実際にどれくらい稼げる？リアルな収益目安」
5. h2: 「スキルアップに使える厳選ツール・書籍」← Amazonアフィリエイトここに挿入
6. h2: 「よくある質問（FAQ）」
7. まとめ：BOOTHショップへの自然な誘導`,
    },
    {
      name: 'asset_review',
      titleHint: '〇〇の3Dモデル・アセットレビュー・活用法',
      structureGuide: `
【記事構成（アセット紹介・レビュー形式）】
1. 冒頭：「このアセットでゲーム制作がどう変わるか」を先出し（300文字）
2. h2: 「このアセットの特徴・スペック」
3. h2: 「実際の使い方・Unity/UEへの取り込み方」（h3で細分化）
4. h2: 「こんなゲームに向いている！使用例・活用シーン」
5. h2: 「合わせて使いたい厳選アセット・書籍」← Amazonアフィリエイトここに挿入
6. h2: 「よくある質問（FAQ）」
7. まとめ：BOOTHショップへの自然な誘導`,
    },
  ];
...
→ 上記を絡めて「お金・副業・節約・投資・AI活用・仕事効率化・ゲーム開発副業・3DCGコンテンツ販売」のいずれかの角度で書くこと。

【BOOTHショップ紹介ルール（重要）】
記事のテーマが「ゲーム開発」「3Dモデル」「アセット」「ローポリ」Unity・UE5・インディーゲーム「デジタル販売」「BOOTH」「Blender」などに関連する場合、記事の「まとめ」の直前または内部に、以下の文章を必ず自然な形で挿入すること：
...
上記の形式で自然に挿入すること。BOOTHへの誘導は強制的かつ自然に。
